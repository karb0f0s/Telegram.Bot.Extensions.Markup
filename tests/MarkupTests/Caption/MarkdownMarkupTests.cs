using MarkupTests.Fixture;
using Telegram.Bot.Types;

namespace MarkupTests.Caption;

public class MarkdownMarkupTests : IClassFixture<MarkupTestFixture>
{
    private readonly MarkupTestFixture _fixture;

    public MarkdownMarkupTests(MarkupTestFixture fixture)
    {
        _fixture = fixture;
    }

    [Fact]
    public void Test_caption_markdown_simple()
    {
        string test_md_string = """
            Test for <*bold*, _ita_\__lic_, `code`,
             [links](http://github.com/ab_),
             [text-mention](tg://user?id=123456789) and ```python\\npre```.
             http://google.com/ab\_
            """
            .ReplaceLineEndings("")
            .Replace(@"\\n", "\n", StringComparison.Ordinal);

        string? text_markdown = _fixture.TestMessage.CaptionMarkdown();

        Assert.Equal(test_md_string, text_markdown);
    }

    [Fact]
    public void Test_caption_markdown_v2_simple()
    {
        string test_md_string = """
            __Test__ for <*bold*, _ita\_lic_, `\\\`code`,
             [links](http://github.com/abc\\\)def),
             [text\-mention](tg://user?id=123456789) and ```\`\\pre```\.
             http://google\.com and _bold *nested in ~strk\>trgh~ nested in* italic_\.
             ```python\\nPython pre```\. ||Spoiled||\.
            """
            .ReplaceLineEndings("")
            .Replace(@"\\n", "\n", StringComparison.Ordinal);

        string? text_markdown = _fixture.TestMessageV2.CaptionMarkdownV2();

        Assert.Equal(test_md_string, text_markdown);
    }

    [Fact]
    public void Test_caption_markdown_empty()
    {
        Message message = new()
        {
            Text = "test",
            Caption = null,
        };

        Assert.Null(message.CaptionMarkdown());
        Assert.Null(message.CaptionMarkdownV2());
    }

    [Fact]
    public void Test_caption_markdown_urled()
    {
        string test_md_string = """
            Test for <*bold*, _ita_\__lic_, `code`,
             [links](http://github.com/ab_),
             [text-mention](tg://user?id=123456789) and ```python\\npre```.
             [http://google.com/ab_](http://google.com/ab_)
            """
            .ReplaceLineEndings("")
            .Replace(@"\\n", "\n", StringComparison.Ordinal);

        string? text_markdown = _fixture.TestMessage.CaptionMarkdownUrled();

        Assert.Equal(test_md_string, text_markdown);
    }

    [Fact]
    public void Test_caption_markdown_v2_urled()
    {
        string test_md_string = """
            __Test__ for <*bold*, _ita\_lic_, `\\\`code`,
             [links](http://github.com/abc\\\)def),
             [text\-mention](tg://user?id=123456789) and ```\`\\pre```\.
             [http://google\.com](http://google.com) and _bold *nested in ~strk\>trgh~
             nested in* italic_\. ```python\\nPython pre```\. ||Spoiled||\.
            """
            .ReplaceLineEndings("")
            .Replace(@"\\n", "\n", StringComparison.Ordinal);

        string? text_markdown = _fixture.TestMessageV2.CaptionMarkdownV2Urled();

        Assert.Equal(test_md_string, text_markdown);
    }
}
